library(igraph)
library(readr)
DEMtweets <- read.csv("Desktop/MasterDEMOCRAT.csv", header = TRUE)
...
library(tm)
tweets_source <- VectorSource(DEMtweets_text)
...
frequent_terms <- freq_terms(DEMtweets_text, 100)
plot(frequent_terms)
...
f_clean_tweets <- function (DEMtweets) {
  
  clean_tweets = DEMtweets$tweet_text
...
clean_tweets <- f_clean_tweets(DEMtweets)
...
plot_ly(emo_sum, x=~emotion, y=~count, type="bar", color=~emotion) %>%
  layout(xaxis=list(title=""), showlegend=FALSE,
         title="Distribution of emotion categories for DEMOCRATIC Tweets")
